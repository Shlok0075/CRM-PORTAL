...
export default function Dashboard() {
  const tasksApi = useApi('/tasks?limit=50')
  const clientsApi = useApi('/clients?limit=50')
  const complianceApi = useApi('/compliance/upcoming')
  const invoicesApi = useApi('/finance/invoices?limit=50')

  const tasks = tasksApi.data?.data || tasksApi.data || []
  const clients = clientsApi.data?.data || clientsApi.data || []
  const compliance = complianceApi.data?.data || complianceApi.data || []
  const invoices = invoicesApi.data?.data || invoicesApi.data || []

  const loading = tasksApi.loading || clientsApi.loading || complianceApi.loading || invoicesApi.loading
  const error = tasksApi.error || clientsApi.error || complianceApi.error || invoicesApi.error
  ...
  const overdueTasks = tasks.filter((t: any) => t.isOverdue || t.status === 'overdue').length
  ...
  const stats = [
    { label: 'Total Clients', value: clients.length, icon: Users, color: 'from-blue-500 to-cyan-400', bg: 'bg-blue-50', text: 'text-blue-600', change: `+${clients.length}`, up: true },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredTasks.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400 text-sm">No tasks found</td></tr>
                  ...
                  <tr key={task.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className={`px-4 py-3 text-sm ${task.isOverdue ? 'text-red-600 font-medium' : 'text-gray-600'}`}>
                      {task.dueDate ? new Date(task.dueDate).toLocaleDateString('en-IN') : '-'}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                        task.status === 'completed' || task.status === 'verified' ? 'bg-emerald-50 text-emerald-700' :
                        task.status === 'in_progress' ? 'bg-blue-50 text-blue-700' :
                        task.isOverdue || task.status === 'overdue' ? 'bg-red-50 text-red-700' :
                        'bg-amber-50 text-amber-700'
                      }`}>
                        {task.isOverdue && <AlertCircle size={12} className="mr-1" />}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Pending', value: pendingTasks, color: 'bg-amber-50 text-amber-700', border: 'border-amber-200' },
              { label: 'In Progress', value: inProgressTasks, color: 'bg-blue-50 text-blue-700', border: 'border-blue-200' },
              { label: 'Completed', value: completedTasks, color: 'bg-emerald-50 text-emerald-700', border: 'border-emerald-200' },
              { label: 'Overdue', value: overdueTasks, color: 'bg-red-50 text-red-700', border: 'border-red-200' },
            ].map((item) => (
          ...
          {taskStatusData.length > 0 && (
            <div className="mt-4">
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={taskStatusData} cx="50%" cy="50%" outerRadius={70} paddingAngle={4} dataKey="value">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-3">
            {compliance.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">No compliance entries yet</p>
            ) : compliance.slice(0, 5).map((item: any) => (
              <div key={item.id} className="flex items-center gap-3 p-3 bg-gray-50/80 rounded-xl hover:bg-indigo-50/50 transition-colors">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-md">
                  {item.name?.charAt(0) || 'C'}
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-sm text-gray-900">{item.name}</p>
                  <p className="text-xs text-gray-500">{item.applicableTo}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-gray-700">{item.dueDateRule}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {invoices.length === 0 ? (
              ...
              <div key={inv.id} className="flex items-center justify-between p-3 bg-gray-50/80 rounded-xl">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-gray-900">₹{inv.total?.toLocaleString('en-IN') || '0'}</p>
                  <span className={`text-xs ${inv.status === 'paid' ? 'text-emerald-600' : inv.status === 'overdue' ? 'text-red-600' : 'text-amber-600'}`}>{inv.status}</span>
                </div>
              </div>
            ))}
          </div>
